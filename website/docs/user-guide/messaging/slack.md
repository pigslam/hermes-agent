...
sidebar_position: 4
title: "Slack"
description: "Set up Reuben Agent as a Slack bot using Socket Mode"
---

# Slack Setup

Connect Reuben Agent to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
...
4. Enter an app name (e.g., "Reuben Agent") and select your workspace
5. Click **Create App**
...
| **Channels** | Bot **only responds when @mentioned** (e.g., `@Reuben Agent what time is it?`). In channels, Hermes replies in a thread attached to that message. |
...
Make sure the bot has been **invited to the channel** (`/invite @Reuben Agent`).
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @Reuben Agent` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @Reuben Agent` |
...
7. ✅ Bot was **invited** to the channel (`/invite @Reuben Agent`)
