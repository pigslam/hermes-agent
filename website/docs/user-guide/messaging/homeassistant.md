...
description: Control your smart home with Reuben Agent via Home Assistant integration.
...
Reuben Agent integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "Reuben Agent"
5. Copy the token
...
Reuben Agent registers four tools for smart home control:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "Reuben Agent".
