...
Reuben Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.
...
| `retain_context` | `conversation between Reuben Agent and the User` | Context label for retained memories |
