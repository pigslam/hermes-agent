...
This guide walks you through connecting Reuben Agent to GitHub so it automatically fetches a pull request's diff, analyzes the code changes, and posts a comment — triggered by a webhook event with no manual prompting.
...
- Reuben Agent installed and running (`hermes gateway`)
