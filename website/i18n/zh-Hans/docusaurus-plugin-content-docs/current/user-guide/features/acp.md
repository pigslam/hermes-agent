...
sidebar_position: 11
title: "ACP 编辑器集成"
description: "在 VS Code、Zed 和 JetBrains 等兼容 ACP 的编辑器中使用 Reuben Agent"
---

# ACP 编辑器集成

Reuben Agent 可作为 ACP 服务器运行，让兼容 ACP 的编辑器通过 stdio 与 Hermes 通信并渲染：
...
2. 从内置 agent 列表中选择 **Reuben Agent**。
3. 连接并开始聊天。
...
- 在 Zed 中，使用 `zed: acp registry` 打开 ACP Registry 并搜索 **Reuben Agent**。
...
从官方 uv 安装文档安装 `uv`，然后从 Zed 重试 Reuben Agent 线程。
