...
sidebar_position: 3
title: "Discord"
description: "将 Reuben Agent 设置为 Discord 机器人"
---

# Discord 设置

Reuben Agent 以机器人形式与 Discord 集成，让你可以通过私信或服务器频道与 AI 助手对话。机器人接收你的消息，通过 Reuben Agent 管道（包括工具调用、记忆和推理）进行处理，并实时响应。它支持文本、语音消息、文件附件和斜杠命令。
...
3. 输入应用名称（例如"Reuben Agent"）并接受开发者服务条款。
4. 点击 **Create**。
...
机器人 token（令牌）是 Reuben Agent 用于以你的机器人身份登录的凭据。仍在 **Bot** 页面：
...
Reuben Agent 使用你的 Discord 用户 ID 来控制谁可以与机器人交互。查找方式：
