...
sidebar_position: 4
title: "Slack"
description: "使用 Socket Mode 将 Reuben Agent 设置为 Slack 机器人"
---

# Slack 设置

使用 Socket Mode 将 Reuben Agent 作为机器人连接到 Slack。Socket Mode 使用 WebSocket 而非公开 HTTP 端点，因此你的 Hermes 实例无需公开访问——它可以在防火墙后、笔记本电脑上或私有服务器上正常运行。
...
2. 点击 **Create New App**
3. 选择 **From scratch**
4. 输入应用名称（例如 "Reuben Agent"）并选择你的工作区
5. 点击 **Create App**
...
|---------|----------|
| **私信** | 机器人响应每条消息——无需 @ 提及 |
| **频道** | 机器人**仅在被 @ 提及时响应**（例如 `@Reuben Agent what time is it?`）。在频道中，Hermes 在该消息附带的话题中回复。 |
...
确保机器人已被**邀请到该频道**（`/invite @Reuben Agent`）。
...
| 机器人在私信中正常但在频道中不响应 | **最常见问题。** 将 `message.channels` 和 `message.groups` 添加到事件订阅，重新安装应用，并用 `/invite @Reuben Agent` 邀请机器人加入频道 |
...
| 机器人响应但无法在频道中发帖 | 用 `/invite @Reuben Agent` 邀请机器人加入频道 |
...
7. ✅ 已**邀请**机器人加入频道（`/invite @Reuben Agent`）
