...
sidebar_position: 7
title: "Docker"
description: "在 Docker 中运行 Reuben Agent 以及将 Docker 用作终端后端"
---

# Reuben Agent — Docker

Docker 与 Reuben Agent 的交集有两种截然不同的方式：
...
如果这是你第一次运行 Reuben Agent，请在宿主机上创建一个数据目录，并以交互方式启动容器以运行设置向导：
...
如果某个工具可能对大多数 Reuben Agent 用户有用，考虑将其贡献到上游，而不是在私有派生镜像中维护。在 [hermes-agent 仓库](https://github.com/NousResearch/hermes-agent)提交 issue 或 pull request，描述该工具及其使用场景。被纳入官方镜像的工具惠及所有用户，并避免了维护下游 fork 的开销。
