...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Reuben Agent\",
    \"comments\": [
...
```bash
# 若无问题——批准
gh pr review $PR_NUMBER --approve --body "Reviewed by Reuben Agent. Code looks clean — good test coverage, no security concerns."
