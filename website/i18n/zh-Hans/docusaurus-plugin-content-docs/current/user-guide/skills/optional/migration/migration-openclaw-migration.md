---
title: "Openclaw Migration — 将用户的 OpenClaw 自定义配置迁移到 Reuben Agent"
sidebar_label: "Openclaw Migration"
description: "将用户的 OpenClaw 自定义配置迁移到 Reuben Agent"
---
...
将用户的 OpenClaw 自定义配置迁移到 Reuben Agent。从 `~/.openclaw` 导入 Hermes 兼容的记忆、`SOUL.md`、命令白名单、用户技能及所选工作区资产，并精确报告无法迁移的内容及原因。
...
当用户希望以最少的手动清理将其 OpenClaw 配置迁移到 Reuben Agent 时，使用此 skill。
