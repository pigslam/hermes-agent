...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or reuben -c.",
    ...
    # --- CLI Flags ---
    "reuben -c resumes your most recent CLI session. reuben -c \"project name\" resumes by title.",
    "reuben -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "reuben -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "reuben chat -t web,terminal enables only specific toolsets for a focused session.",
    "reuben chat -s github-pr-workflow preloads a skill at launch.",
    "reuben chat -q \"query\" runs a single non-interactive query and exits.",
    "reuben chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "reuben chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "reuben --yolo bypasses all dangerous command approval prompts for the entire session.",
    "reuben chat --source telegram tags the session for filtering in reuben sessions list.",
    "reuben -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "reuben doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "reuben dump outputs a compact setup summary — great for bug reports.",
    "reuben config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "reuben config edit opens config.yaml in your default editor.",
    "reuben config check scans for missing or stale configuration options.",
    "reuben sessions browse opens an interactive session picker with search.",
    "reuben sessions stats shows session counts by platform and database size.",
    "reuben sessions prune --older-than 30 cleans up old sessions.",
    "reuben skills search react --source skills-sh searches the skills.sh public directory.",
    "reuben skills check scans installed hub skills for upstream updates.",
    "reuben skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "reuben skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "reuben mcp add github --command npx adds MCP servers from the command line.",
    "reuben mcp serve runs Hermes itself as an MCP server for other agents.",
    "reuben auth add lets you add multiple API keys for credential pool rotation.",
    "reuben completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "reuben logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "reuben backup creates a zip backup of your entire Hermes home directory.",
    "reuben profile create coder creates an isolated profile that becomes its own command.",
    "reuben profile create work --clone copies your current config and keys to a new profile.",
    "reuben update syncs new bundled skills to ALL profiles automatically.",
    "reuben gateway install sets up Hermes as a system service (systemd/launchd).",
    "reuben memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "reuben webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: reuben tools disables unused tools, reuben skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "reuben models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as reuben -w).",
    ...
    "Profile names become shell commands — 'reuben profile create coder' creates the 'coder' command.",
    "reuben profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "reuben sessions export backup.jsonl exports all sessions for backup or analysis.",
    "reuben -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "reuben skills install official/security/1password installs optional skills from the repo.",
    ...
    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: reuben cron add --skill blogwatcher \"Check for new posts\".",
    ...
    "reuben gateway install sets it up as a system service that starts on boot.",
    ...
    # --- MCP ---
    "reuben mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "reuben mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "reuben mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — reuben tools can toggle them per platform.",
    ...
    "reuben chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "reuben plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "reuben acp runs Hermes as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "reuben auth add nous or reuben auth add openai-codex sets up OAuth-based providers.",
    ...
    "reuben status --deep runs deeper diagnostic checks across all components.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'reuben profile create ls' would be rejected.",
    "reuben profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "reuben claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via reuben tools.",
    ...
    # --- Curator ---
    'reuben curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "reuben curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'reuben curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'reuben auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    'reuben dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'reuben dashboard embeds the full Hermes TUI in your browser via xterm.js and a WebSocket PTY.',
    ...
    'reuben -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'reuben chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'reuben chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'reuben chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "reuben chat --source tool tags programmatic chats so they don't clutter reuben sessions list.",
    'reuben dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'reuben sessions rename <ID> "new title" renames any past session; reuben sessions delete <ID> removes one.',
    'reuben import restores a session export or profile archive produced by sessions export or profile export.',
    'reuben fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'reuben pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'reuben setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'reuben status --deep runs the full health sweep across every component; plain reuben status is the quick view.',
    ...
    '--source tool chats are excluded from reuben sessions list by default — set --source explicitly to see them.',
