"""CLI for the Hermes Kanban board — ``reuben kanban …`` subcommand.
...
def _check_dispatcher_presence() -> tuple[bool, str]:
    ...
    Used by ``reuben kanban create`` (and callers) to warn when a task
    ...
    if pid and not dispatch_on:
        return (
            ...
            "standalone daemon (`reuben kanban daemon --force`)."
        )
    return (
        False,
        "No gateway is running — the task will sit in 'ready' until you "
        "start it. Run:\n"
        "    reuben gateway start\n"
...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    kanban_parser.add_argument(
        ...
        help=(
            ...
            "(set via `reuben kanban boards switch <slug>` or the "
            "HERMES_KANBAN_BOARD env var). Use `reuben kanban boards list` "
            "to see all boards."
        ),
    )
    ...
    p_create.add_argument("--project", default=None,
                          help="Link to a project (id or slug). Anchors the task's "
                               ...
                               "deterministic branch. See `reuben project list`.")
    ...
    p_daemon = sub.add_parser(
        "daemon",
        help="DEPRECATED — dispatcher now runs in the gateway. Use `reuben gateway start`.",
    )
...
def kanban_command(args: argparse.Namespace) -> int:
    """Entry point from ``reuben kanban …`` argparse dispatch.
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print(
                "usage: reuben kanban <action> [options]\n"
                "Run 'reuben kanban --help' for the full list of actions.",
                file=sys.stderr,
            )
        return 0
    ...
    if board_override:
        ...
        if normed != kb.DEFAULT_BOARD and not kb.board_exists(normed):
            print(
                ...
                f"Create it with `reuben kanban boards create {normed}`.",
                file=sys.stderr,
            )
            return 1
...
# ---------------------------------------------------------------------------
# Boards management (reuben kanban boards …)
# ---------------------------------------------------------------------------

def _dispatch_boards(args: argparse.Namespace) -> int:
    """Handle ``reuben kanban boards <action>``.
...
def _cmd_boards_list(args: argparse.Namespace) -> int:
    ...
    if not boards:
        print("(no boards — create one with `reuben kanban boards create <slug>`)")
        return 0
    ...
    if len(boards) > 1:
        print("Switch boards with `reuben kanban boards switch <slug>`.")
    return 0
...
def _cmd_boards_create(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "switch", False):
        ...
        print(f"  Use `reuben kanban boards switch {meta['slug']}` to make it current.")
    return 0


def _cmd_boards_rm(args: argparse.Namespace) -> int:
    # When the user runs `reuben kanban boards delete <slug>` (alias), the
...
def _cmd_init(args: argparse.Namespace) -> int:
    ...
    if profiles:
        ...
        print("Create one with `reuben -p <name> setup` before assigning tasks.")
    print()
    print("Next step: start the gateway so ready tasks actually get picked up.")
    print("  reuben gateway start")
    print()
    print(
...
def _cmd_assignees(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "json", False):
        ...
        return 0
    if not data:
        print("(no assignees — create a profile with `reuben -p <name> setup`)")
        return 0
    # Header
...
def _cmd_show(args: argparse.Namespace) -> int:
    ...
    if task.result:
        ...
        # Surface it at top-level so a glance at ``reuben kanban show <id>``
...
def _cmd_daemon(args: argparse.Namespace) -> int:
    ...
    if not getattr(args, "force", False):
        print(
            "reuben kanban daemon: DEPRECATED — the dispatcher now runs\n"
            "inside the gateway. To use kanban:\n"
            "\n"
            "    reuben gateway start       # starts the gateway + embedded dispatcher\n"
            "\n"
        ...
        if health_state["bad_ticks"] >= HEALTH_WINDOW:
            ...
            if now - health_state["last_warn_at"] >= 300:
                print(
                    ...
                    f"`reuben kanban list --status ready` / "
                    f"`reuben kanban list --status blocked` for recent "
