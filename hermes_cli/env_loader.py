...
# Used by setup / `reuben model` flows to label detected credentials so
...
def _sanitize_loaded_credentials() -> None:
    ...
    for key, value in list(os.environ.items()):
        ...
        print(
            ...
            "  provider's dashboard and run `reuben setup` (or edit the "
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    if result.applied:
        ...
        # edited .env (see #6843).
        _sanitize_loaded_credentials()
        # Remember where these came from so the setup / `reuben model`
