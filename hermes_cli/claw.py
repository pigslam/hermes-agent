"""reuben claw — OpenClaw migration commands.

Usage:
    reuben claw migrate              # Preview then migrate (always shows preview first)
    reuben claw migrate --dry-run    # Preview only, no changes
    reuben claw migrate --yes        # Skip confirmation prompt
    reuben claw migrate --preset full --overwrite --migrate-secrets  # Full run w/ secrets
    reuben claw migrate --no-backup  # Skip pre-migration snapshot
    reuben claw cleanup              # Archive leftover OpenClaw directories
    reuben claw cleanup --dry-run    # Preview what would be archived
"""
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    ...
    print_info("Recommendation: stop the gateway first with 'reuben gateway stop'.")
    print()
...
def claw_command(args):
    ...
    if action == "migrate":
        ...
        print("Usage: reuben claw <command> [options]")
        print()
        print("Commands:")
        ...
        print()
        print("Run 'reuben claw <command> --help' for options.")
...
def _cmd_migrate(args):
    ...
    if not source_dir.is_dir():
        ...
        print_info("You can specify a custom path: reuben claw migrate --source /path/to/.openclaw")
        return
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info("To execute, re-run with: reuben claw migrate --yes")
            return
    ...
    # restorable with `reuben import`.  Mirrors OpenClaw's
    ...
    if not no_backup:
        try:
            ...
            if backup_archive:
                ...
                print_info(f"Restore with: reuben import {backup_archive.name}")
        except Exception as e:
            print()
    ...
    # 'reuben claw cleanup' separately.
...
def _cmd_cleanup(args):
    ...
    if running:
        ...
        if not auto_yes:
            ...
            if not prompt_yes_no("Proceed anyway?", default=False):
                print_info("Aborted. Stop OpenClaw first, then re-run: reuben claw cleanup")
                return
    ...
    for source_dir in dirs_to_check:
        ...
        if dry_run:
            ...
            print_info("To execute, re-run with: reuben claw cleanup --yes")
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  reuben claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info("  reuben claw migrate --migrate-secrets")
            print()
            print_info("Or add your key manually:")
            print_info("  reuben config set OPENROUTER_API_KEY sk-or-v1-...")
