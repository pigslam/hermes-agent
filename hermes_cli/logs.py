"""``reuben logs`` — view and filter Reuben log files.
...
Usage examples::
    ...
    reuben logs                    # last 50 lines of agent.log
    reuben logs -f                 # follow agent.log in real time
    reuben logs errors             # last 50 lines of errors.log
    reuben logs gateway -n 100    # last 100 lines of gateway.log
    reuben logs gui -f            # follow gui.log (dashboard/pty/ws)
    reuben logs desktop -f        # follow desktop.log (Electron app boot/backend)
    reuben logs --level WARNING    # only WARNING+ lines
    reuben logs --session abc123   # filter by session ID substring
    reuben logs --component tools  # only tool-related lines
    reuben logs --since 1h         # lines from the last hour
    reuben logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    if not log_path.exists():
        ...
        print(f"(Logs are created when Hermes runs — try 'reuben chat' first)")
        sys.exit(1)
...
def list_logs() -> None:
    ...
    if not found:
        print("  (no log files yet — run 'reuben chat' to generate logs)")
