"""
Unified self-relaunch for the Reuben Agent CLI.

Preserves critical flags (--tui, --dev, --profile, --model, etc.) across
process replacement so that ``reuben sessions browse`` or post-setup relaunch
doesn't silently drop the user's UI mode or other preferences.

Also works when ``reuben`` is not on PATH (e.g. ``nix run`` or ``python -m``).
"""
...
from hermes_cli.branding import CLI_COMMAND, LEGACY_CLI_COMMAND, PRODUCT_NAME
...
def resolve_hermes_bin() -> Optional[str]:
    """Find the current CLI entry point.

    Priority:
      1. ``sys.argv[0]`` if it resolves to a real executable.
      2. ``shutil.which("reuben")`` on PATH.
      3. ``shutil.which("hermes")`` on PATH as a compatibility alias.
    ...
    ``reuben.exe``/``hermes.exe`` on PATH or the ``sys.executable -m hermes_cli.main``
    fallback.
    """
    argv0 = sys.argv[0]
    ...
    # PATH lookup
    for command in (CLI_COMMAND, LEGACY_CLI_COMMAND):
    ...
    *emulates* exec by spawning the child and exiting the parent, but
    ...
    if sys.platform == "win32":
        ...
        try:
            ...
            print(
                f"\n{PRODUCT_NAME} relaunch failed: {exc}\n"
                f"Command: {' '.join(new_argv)}\n"
                f"Fix: open a new terminal so PATH picks up, then re-run {CLI_COMMAND}.",
                file=sys.stderr,
            )
            sys.exit(1)
    else:
