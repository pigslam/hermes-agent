...
Handles: reuben gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# when hermes_constants is cached from a pre-update version during `reuben update`.
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``reuben gateway status`` from falsely counting the ``hermes`` CLI that
...
def _scan_gateway_pids(
    ...
    # scan (e.g. ``reuben gateway status``) is never mistaken for a running
    # gateway.  See #13242.
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``reuben update``
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    # when the user closes the terminal, leaving ``reuben update`` users
    # with no running gateway until they re-invoke ``reuben gateway``
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}reuben gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(
        f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}reuben gateway restart{scope_flag}"
    )
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    if _launchd_unsupported_marker_exists():
        ...
        print("  Stop it with: reuben gateway stop")
    else:
        print(
            "⚠ Gateway process is running for this profile, but the service is not active"
        )
        print(f"  PID(s): {_format_gateway_pids(snapshot.gateway_pids, limit=None)}")
        print("  This is usually a manual foreground/tmux/nohup run, so `reuben gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``reuben gateway status`` output so users with
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``reuben gateway run`` must remain interruptible from
...
SERVICE_DESCRIPTION = "Reuben Agent Gateway - Messaging Platform Integration"
...
def print_legacy_unit_warning() -> None:
    ...
    print_info("    reuben gateway migrate-legacy")
...
def remove_legacy_hermes_units(
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: reuben gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo reuben gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    reuben gateway uninstall")
    print_info("    sudo reuben gateway uninstall --system")
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            ...
            print_warning(
                ...
                "root shell, or install a user service instead: reuben gateway install"
            )
            return scope, False
...
def _stable_service_working_dir() -> str:
    ...
    dir, or a clone that ``reuben update`` later relocates/removes — the path
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    ...
    Seen live 2026-06-11: an E2E guard probe ran ``reuben gateway restart``
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo reuben gateway uninstall --system")
    print_info("         reuben gateway install")
    print_info("         reuben gateway start")
...
def systemd_install(
    ...
    print("Next steps:")
    print(
        f"  {'sudo ' if system else ''}reuben gateway start{scope_flag}              # Start the service"
    )
    print(
        f"  {'sudo ' if system else ''}reuben gateway status{scope_flag}             # Check status"
    )
    print(
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}reuben gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_stop(system: bool = False):
    ...
    try:
        ...
        print(
            ...
            "check `reuben gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `reuben gateway status` or logs for final state."
            )
            return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(
            f"  Run: {'sudo ' if system else ''}reuben gateway restart{scope_flag}  # auto-refreshes the unit"
        )
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}reuben gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(
            f"  Run after the start-limit window expires: {'sudo ' if system else ''}reuben gateway restart{scope_flag}"
        )
        print(
        ...
        print(
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}reuben gateway start{scope_flag}"
        )
...
# process invocation (each ``reuben gateway start/stop/status`` call).
...
# (the documented `nohup reuben gateway run` workaround). See #23387.
...
def _spawn_detached_gateway() -> bool:
    ...
    macOS 26+ (issue #23387). Mirrors the `nohup reuben gateway run --replace`
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    print(
        f"  Try manually: nohup reuben gateway run --replace "
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  reuben gateway status             # Check status")
    from hermes_constants import display_hermes_home as _dhh
...
def launchd_stop():
    ...
    # `reuben gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        ...
        print("  Run: reuben gateway start")
    ...
    if service_listed:
        if launchd_pid is not None:
            ...
            if fallback_pid:
                ...
                print("  Cron jobs will fire. Stop with: reuben gateway stop")
            else:
...
def _guard_named_profile_under_multiplexer(force: bool = False) -> None:
    ...
    named-profile ``reuben gateway run`` is always a misconfiguration, so we
...
def _guard_supervised_gateway_conflict(force: bool = False) -> None:
    ...
    Running ``reuben gateway run [--replace]`` (or the manual-restart fallback)
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    ``reuben gateway run`` can burn memory/CPU just to fail with "already
    ...
    print("  Use 'reuben gateway restart' to replace it,")
    print("  or 'reuben gateway stop' first.")
    print("  Or use 'reuben gateway run --replace' to auto-replace.")
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # from sibling CLI processes, but foreground `reuben gateway run` still
    ...
    # /restart) rather than through `reuben gateway restart` which already
    ...
    # `reuben gateway start/restart` — leaving the gateway vulnerable to
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "6. To authorize users, use DM pairing: reuben pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Reuben will connect automatically over WebSocket",
        ],
        "vars": [
            {
                "name": "YUANBAO_APP_ID",
...
def _all_platforms() -> list[dict]:
    ...
    ``reuben setup gateway`` without needing the gateway to be running.
            ...
            if value:
                ...
                if is_email:
                    ...
                    access_choices = [
                        ...
                        "Use DM pairing (unknown users request access, you approve with 'reuben pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info(
                        "  Approve with: reuben pairing approve <platform> <code>"
                    )
                elif is_email:
                    print_success("  Unknown email senders will be ignored.")
                else:
                    print_info(
                        "  Skipped — configure later with 'reuben gateway setup'"
                    )
            continue
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Reuben will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info(
        "  3. Reuben will store the returned account_id/token in ~/.hermes/.env."
    )
    print_info(
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `reuben gateway setup`.")
        return
    ...
    if access_idx == 0:
        ...
        print_info(
            "  Unknown DM users can request access and you approve them with `reuben pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_qqbot():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info(
            "  Unknown users can request access; approve with `reuben pairing approve`."
        )
    elif access_idx == 1:
...
def gateway_setup():
    ...
    if any_configured:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: reuben gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    try:
                        ...
                        print_info("  You can try manually: reuben gateway install")
                else:
                    print_info("  Skipped start and auto-start setup.")
                    print_info("  You can install later: reuben gateway install")
                    if supports_systemd_services():
                        print_info(
                            "  Or as a boot-time service: sudo reuben gateway install --system"
                        )
                    print_info("  Or run in foreground:  reuben gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: reuben gateway run")
                print_info(
                    "  For persistence:   tmux new -s hermes 'reuben gateway run'"
                )
                print_info(
                ...
                print_info("  Run in foreground: reuben gateway run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup reuben gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
                )
            else:
        ...
        print()
        print_info("No platforms configured. Run 'reuben gateway setup' when ready.")
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``reuben gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``reuben gateway install|uninstall|start|stop|restart``
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: reuben gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning(
                ...
                print_info(
                    "  Consider running in foreground instead: reuben gateway run"
                )
                print_info(
                    "  Or use tmux/screen for persistence: tmux new -s hermes 'reuben gateway run'"
                )
                print()
            ...
            print()
            print(
                "  reuben gateway run                              # direct foreground"
            )
            print(
                "  tmux new -s hermes 'reuben gateway run'         # persistent via tmux"
            )
            print(
                "  nohup reuben gateway run > ~/.hermes/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  reuben profile create <name>     # creates the s6 service slot")
                print("  reuben -p <name> gateway start   # bring it up via s6")
                print("  reuben status                    # see currently-supervised gateways")
                return
            ...
            print()
            print("To run the gateway: reuben gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: reuben gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: reuben gateway stop")
            sys.exit(1)
        if supports_systemd_services():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  reuben profile delete <name>     # tears down the s6 service slot")
                print("  reuben -p <name> gateway stop    # stop without deleting the profile")
                return
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: reuben gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if os.getenv("_HERMES_GATEWAY") == "1":
            print_error(
                ...
                "Use `reuben gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if os.getenv("_HERMES_GATEWAY") == "1":
            print_error(
                ...
                "This command was blocked to prevent restart loops.\n"
                "Use `reuben gateway restart` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: reuben gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  reuben gateway install")
                else:
                    print("To install as a service:")
                    print("  reuben gateway install")
                    print("  sudo reuben gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  reuben gateway run      # Run in foreground")
                if is_termux():
                    print(
                        "  nohup reuben gateway run > ~/.hermes/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s hermes 'reuben gateway run'         # persistent via tmux"
                    ...
                    print(
                        "  reuben gateway install  # Install as Windows Scheduled Task (auto-start on login)"
                    )
                else:
                    print("  reuben gateway install  # Install as user service")
                    print(
                        "  sudo reuben gateway install --system  # Install as boot-time system service"
                    )
