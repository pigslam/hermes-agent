...
entries via ``reuben mcp catalog`` or the interactive ``reuben mcp picker``,
and install them with ``reuben mcp install <name>`` (or by toggling in the
...
- Manifests pin transport details (commands, args, refs). MCPs are never
  auto-updated; users explicitly re-run ``reuben mcp install <name>`` to
...
def _apply_tool_selection(
    ...
    Probe-fail path:
      ...
      - Either way, point the user at ``reuben mcp configure <name>``.
    """
    print()
    ...
    if probed is None:
        ...
        if manifest_default:
            ...
            print(color(
                ...
                f"Run `reuben mcp configure {entry.name}` after the server "
            ...
            print(color(
                ...
                f"Run `reuben mcp configure {entry.name}` after first "
                "connect to prune.",
                Colors.YELLOW,
            ))
        return
    ...
    if not chosen_indices:
        ...
        print(color(
            f"  No tools selected. Run `reuben mcp configure {entry.name}` "
            "to change.",
            Colors.YELLOW,
        ))
        return
    ...
    if len(chosen_indices) == len(probed):
        ...
        # the user can re-run `reuben mcp configure <name>` and unselect a
...
def install_entry(entry: CatalogEntry, *, enable: bool = True) -> None:
    ...
    if entry.auth.type == "api_key":
        ...
        if entry.auth.provider:
            ...
            # the existing `reuben auth <provider>` flow. Surface guidance
            ...
            print(color(
                ...
                f"`reuben auth {entry.auth.provider}` if you have not "
