"""reuben hooks — inspect and manage shell-script hooks.

Usage::

    reuben hooks list
    reuben hooks test <event> [--for-tool X] [--payload-file F]
    reuben hooks revoke <command>
    reuben hooks doctor
...
def hooks_command(args) -> None:
    """Entry point for ``reuben hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: reuben hooks {list|test|revoke|doctor}")
        print("Run 'reuben hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        ...
        print("See `reuben hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `reuben hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `reuben hooks test` and `reuben hooks doctor`
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `reuben hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `reuben hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `reuben hooks doctor`.")
