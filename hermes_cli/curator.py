"""CLI subcommand: `reuben curator <subcommand>`.
...
def _cmd_run(args) -> int:
    ...
    if not synchronous:
        print("llm pass running in background — check `reuben curator status` later")
    if dry:
        if synchronous:
            print(
                "dry-run: no changes applied. Read the report with "
                "`reuben curator status` and run `reuben curator run` (no flag) to apply."
            )
        else:
            print(
...
def _cmd_archive(args) -> int:
    ...
    if skill_usage.get_record(args.skill).get("pinned"):
        print(
            ...
            f"`reuben curator unpin {args.skill}`"
        )
        return 1
    ...
    if target_path is None:
        ...
        if not rows:
            print(
                ...
                "`reuben curator backup` or wait for the next curator run."
            )
        else:
            print(
