"""
Interactive setup wizard for Reuben Agent.
...
# display_hermes_home imported lazily at call sites (stale-module safety during reuben update)
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Reuben Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    ...
    print_info("  reuben config set model.provider custom")
    print_info("  reuben config set model.base_url http://localhost:8080/v1")
    print_info("  reuben config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'reuben setup' in an interactive terminal to use the full wizard.")
    print()
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'reuben setup')")
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'reuben setup' to configure"))
    ...
    # Video generation — opt-in via `reuben tools` → Video Generation.
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'reuben setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'reuben setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'reuben setup terminal'"))
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'reuben setup tools' to configure them,"
        )
    ...
    print()
    print(f"   {color('reuben setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('reuben setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('reuben setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('reuben setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('reuben setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('reuben config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('reuben config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('reuben config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('reuben', Colors.GREEN)}              Start chatting")
    print(f"   {color('reuben gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('reuben doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `reuben tools` and `reuben setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``reuben model``)
    ...
    provider added to ``reuben model`` is automatically available here.
    ...
    # Delegate to the shared reuben model flow — handles provider picker,
    ...
    try:
        ...
        print_info("You can try again later with: reuben model")
    ...
    # on demand via `reuben auth add`, `reuben setup` vision, and
    # `reuben setup tts`. This keeps both quick and full setup thin.
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``reuben model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via reuben setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'reuben setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    if selected_backend == "local":
        ...
        # configurable later via `reuben setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `reuben setup terminal`.
...
def _apply_default_agent_settings(config: dict):
    ...
    print_info("  Run `reuben setup agent` later to customize.")
...
def _setup_webhooks():
    ...
    print()
    print_info("   Open config in your editor:  reuben config edit")
    print_info("   Open config in your editor:  reuben config edit")
...
def setup_gateway(config: dict):
    ...
    if not selected:
        print_info("No platforms selected. Run 'reuben setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     reuben config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd and os.geteuid() == 0:  # windows-footgun: ok — guarded by supports_systemd (Linux only)
                    print_info("  Or as a boot-time service: reuben gateway install --system")
                print_info("  Or run in foreground:  reuben gateway")
        else:
            from hermes_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   reuben gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   reuben gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `reuben setup tools` and `reuben tools` use the same flow:
...
def _offer_openclaw_migration(hermes_home: Path) -> bool:
    ...
    print_info("Reuben can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: reuben claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: reuben claw migrate"
        )
        print_info(
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Hermes (use reuben claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``reuben setup --portal`` and ``reuben portal``. This is the
    ...
    ``reuben setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``reuben model`` runs
    when you pick Nous. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``reuben portal`` always offers a model picker,
    ...
    print(color("│     ⚕ Reuben Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    # setup calls, so `reuben portal` == quick setup's Nous step.
    try:
        ...
        print_info("  You can retry later with `reuben portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_nous error during `reuben portal`: %s", exc)
        print()
    ...
    print_info("  Run `reuben portal info` to inspect routing.")
    print_info("  Run `reuben` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      reuben setup           — full or quick (auto-detected)
      reuben setup model     — just model/provider
      reuben setup tts       — just text-to-speech
      reuben setup terminal  — just terminal backend
      reuben setup gateway   — just messaging platforms
      reuben setup tools     — just tool configuration
      reuben setup agent     — just agent settings
    """
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Reuben Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             Reuben Agent Setup Wizard                  │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Reuben Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_info("")
        print_info("Tip: jump straight to a section with 'reuben setup model|terminal|")
    ...
    print()
    print_info("You can edit these files directly or use 'reuben config edit'")
    ...
    # Tune later with `reuben setup agent`.
    if not is_existing:
...
def _run_first_time_quick_setup(config: dict, hermes_home, is_existing: bool):
    ...
    settings, tools); the user can customize later via ``reuben setup <section>``
    or switch providers with ``reuben model``.
    """
    from hermes_cli.config import load_config
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'reuben setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    reuben setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  reuben setup gateway")
    print()
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``reuben tools`` (which
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``reuben setup agent`` /
    ``reuben config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, hermes_home, is_existing: bool):
    ...
    if path == 0:
        ...
        # `reuben update` runs don't re-inject them.
        try:
        ...
        print_info("  Enable tools:        reuben tools")
        print_info("  Seed skills:         reuben skills opt-in --sync")
        print_info("  Add MCP servers:     reuben mcp add")
        print_info("  Enable plugins:      reuben plugins")
        print_info("  Tune agent settings: reuben setup agent")
        print()
        _print_setup_summary(config, hermes_home)
        return
...
def _blank_slate_walkthrough(config: dict, hermes_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            print_info("`reuben update` runs from re-injecting them. Opt back in any")
            print_info("time with `reuben skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `reuben tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `reuben plugins list` / `reuben plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `reuben plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `reuben mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `reuben mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   reuben tools")
    print_info("  Seed skills:         reuben skills opt-in --sync")
    print_info("  Add MCP servers:     reuben mcp add")
    print_info("  Tune agent settings: reuben setup agent")
    print()
...
def _run_quick_setup(config: dict, hermes_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'reuben setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        ...
        print_info("You can configure these later with 'reuben setup gateway'.")
