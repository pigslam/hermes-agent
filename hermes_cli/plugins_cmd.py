"""``reuben plugins`` CLI subcommand — install, update, remove, and list plugins.
        ...
        if target.exists():
            if not force:
                raise PluginOperationError(
                    ...
                    f"or run `reuben plugins update {plugin_name}`.",
                )
...
def cmd_install(
    ...
    if should_enable:
        ...
        console.print(
            ...
            f"Run `reuben plugins enable {installed_name}` to activate.[/dim]",
        )

    console.print("[dim]Restart the gateway for the plugin to take effect:[/dim]")
    console.print("[dim]  reuben gateway restart[/dim]")
    console.print()
...
def _resolve_plugin_key(name: str) -> Optional[str]:
    ...
    This is the single normalization point so ``reuben plugins enable`` /
...
def _filter_plugin_entries(entries: list, args: Any, enabled: set, disabled: set) -> list:
    """Apply ``reuben plugins list`` CLI filters."""
    filtered = entries
...
def cmd_list(args: Any | None = None) -> None:
    ...
    if not entries:
        ...
        console.print("[dim]Install with:[/dim] reuben plugins install owner/repo")
        return
    ...
    console.print(table)
    console.print()
    console.print("[dim]Compact view:[/dim] reuben plugins list --plain --no-bundled")
    console.print("[dim]Interactive toggle:[/dim] reuben plugins")
    console.print("[dim]Enable/disable:[/dim] reuben plugins enable/disable <name>")
...
def plugins_command(args) -> None:
    """Dispatch reuben plugins subcommands."""
