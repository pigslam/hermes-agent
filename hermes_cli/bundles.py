"""Implementation of the ``reuben bundles`` CLI subcommand.
...
def _console() -> Console:
    # Bind to stderr so piping `reuben bundles list | grep …` doesn't
...
def _cmd_list(args) -> None:
    ...
    if not bundles:
        c.print(
            ...
            f"  reuben bundles create <name> --skill skill1 --skill skill2[/]\n"
...
def register_cli(subparser) -> None:
    """Build the ``reuben bundles`` argparse tree.
    ...
    p_create.add_argument(
        ...
        help="Human-readable description shown in /help and `reuben bundles list`",
    )
    p_create.add_argument(
...
def bundles_command(args) -> None:
    """Dispatch ``reuben bundles <subcommand>`` to the right handler."""
