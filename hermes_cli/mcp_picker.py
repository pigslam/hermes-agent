"""MCP picker — interactive `reuben mcp picker` (also the default `reuben mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``reuben mcp add``, lets them pick one, and routes to install / enable /
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `reuben mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: reuben mcp install <name>    Picker: reuben mcp",
        Colors.DIM,
    ))
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Hermes — run `reuben update` "
...
def show_catalog() -> None:
    """`reuben mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`reuben mcp picker` (and default `reuben mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`reuben mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `reuben mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
