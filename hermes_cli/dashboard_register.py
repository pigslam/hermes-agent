"""``reuben dashboard register`` — register a self-hosted dashboard OAuth client.
...
This command:
  1. Resolves a fresh Nous Portal access token from the existing login
     ...
     "run `reuben setup`" hint when the user isn't logged in.
...
def _register_self_hosted_client(
    *,
    ...
    makes re-running ``reuben dashboard register`` idempotent. The portal
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                ...
                "Try `reuben auth add nous` to re-authenticate."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
...
def _print_post_register_hint(
    *,
    ...
    print(
        ...
        "  `reuben dashboard` (localhost) leaves the gate off and serves locally\n"
    ...
    if custom_redirect_uri:
        ...
        print("    reuben dashboard --host 0.0.0.0")
...
def cmd_dashboard_register(args) -> None:
    ...
    if is_managed():
        print(
            "✗ `reuben dashboard register` is not available in a managed/hosted "
            "install.\n"
    ...
    try:
        ...
        if getattr(exc, "relogin_required", False):
            ...
            print("  Run `reuben setup` (or `reuben auth add nous`) first, then retry.")
        else:
