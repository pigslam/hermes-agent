"""``reuben gateway`` and ``reuben proxy`` subcommand parsers.
...
def build_gateway_parser(
    ...
    gateway_enroll = gateway_subparsers.add_parser(
        ...
        description=(
            ...
            "Requires being logged in (reuben setup). Not available in managed installs."
        ),
    )
    ...
    proxy_start.add_argument(
        "--provider",
        default="nous",
        help="Upstream provider: nous or xai (default: nous). See `reuben proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
