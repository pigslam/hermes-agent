"""``reuben debug`` subcommand parser.
...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Reuben Agent. Use 'reuben debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    reuben debug share              Upload debug report and print URL
    reuben debug share --lines 500  Include more log lines
    reuben debug share --expire 30  Keep paste for 30 days
    reuben debug share --local      Print report locally (no upload)
    reuben debug share --no-redact  Disable upload-time secret redaction
    reuben debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'reuben debug share'",
    )
