"""``reuben update`` subcommand parser.
...
def build_update_parser(subparsers, *, cmd_update: Callable) -> None:
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Reuben Agent to the latest version",
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Assume yes for interactive prompts (config migration, stash restore). API-key entry is skipped; run 'reuben config migrate' separately for those.",
    )
    update_parser.add_argument(
        "--branch",
        default=None,
