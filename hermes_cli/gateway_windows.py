...
_TASK_DESCRIPTION = "Reuben Agent Gateway - Messaging Platform Integration"
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    if running_pids:
        ...
        profile_arg = _profile_arg()
        start_cmd = f"reuben {profile_arg} gateway start" if profile_arg else "reuben gateway start"
    ...
    *,
    ...
    if not start_on_login:
        ...
        if start_now:
            ...
            print("  Run later with: reuben gateway start")
        return
        ...
        if prompt_yes_no("  Open the UAC prompt now?", False):
            if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
                ...
                if start_now:
                    ...
                    print("  Approve the Windows UAC prompt, then run: reuben gateway status")
                return
    ...
    if ok:
        ...
        if start_now:
            ...
            print("  Start manually with: reuben gateway start")
        _print_next_steps()
        return
...
def start() -> None:
    ...
    if not task_installed and not startup_installed:
        ...
        if not prompt_yes_no("  Install it now so the gateway starts on login?", True):
            print("  Run: reuben gateway install")
            return
        install(force=False)
        task_installed = is_task_registered()
        startup_installed = is_startup_entry_installed()
        if not task_installed and not startup_installed:
            ...
            print("  If a UAC prompt opened, approve it, then run: reuben gateway start")
            return
...
def restart() -> None:
    ...
    if not _wait_for_gateway_ready(timeout_s=15.0):
        raise RuntimeError(
            ...
            "Check logs/gateway.log and run `reuben gateway status`."
        )
