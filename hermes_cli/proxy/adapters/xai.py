...
class XAIGrokAdapter(UpstreamAdapter):
    ...
    auth_hint = "reuben auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`reuben auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`reuben auth reset xai-oauth` or re-authenticate with "
                    "`reuben auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `reuben auth add xai-oauth --type oauth`."
            )
