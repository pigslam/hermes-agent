"""
MCP Server Management CLI — ``reuben mcp`` subcommand.

Implements ``reuben mcp add/remove/list/test/configure`` for interactive
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    Used after ``reuben mcp login`` to distinguish a genuine authentication
...
def cmd_mcp_add(args):
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  reuben mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  reuben mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  reuben mcp add myserver --preset mypreset')
        return
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            if _save_mcp_server(name, server_config):
                ...
                _info("Fix the issue, then: reuben mcp test " + name)
        return
...
def cmd_mcp_remove(args):
    ...
    # earlier `reuben mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  reuben mcp add <name> --url <endpoint>')
        _info('  reuben mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def _reauth_oauth_server(name: str, server_config: dict) -> bool:
    ...
    landed before reporting success. Shared by ``reuben mcp login`` and
    ``reuben mcp reauth`` so both behave identically for a single server.
    """
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `reuben mcp remove` + `reuben mcp add` to reconfigure auth.")
        return False
    ...
    try:
        ...
        if not _oauth_tokens_present(name):
            ...
            print()
            _info("Then re-run `reuben mcp login " + name + "`.")
            return False
        if tools:
...
def cmd_mcp_reauth(args):
    ...
    ``reuben mcp reauth <name>`` re-auths a single server (same as ``login``).
    ``reuben mcp reauth --all`` discovers every ``auth: oauth`` server in
    ...
    if not name:
        ...
        _info("Usage: reuben mcp reauth <name>   |   reuben mcp reauth --all")
        return
    if name not in servers:
        _error(f"Server '{name}' not found in config.")
        if servers:
...
# ─── reuben mcp configure ────────────────────────────────────────────────────

def cmd_mcp_configure(args):
    """Reconfigure which tools are enabled for an existing MCP server."""
    import sys as _sys
    if not _sys.stdin.isatty():
        print("Error: 'reuben mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``reuben mcp`` subcommands."""
    ...
    if handler:
        ...
        _info("reuben mcp                                    Open the catalog picker (default)")
        _info("reuben mcp catalog                            List Nous-approved MCPs")
        _info("reuben mcp install <name>                     Install a catalog MCP")
        _info("reuben mcp serve                              Run as MCP server")
        _info("reuben mcp add <name> --url <endpoint>        Add a custom MCP server")
        _info("reuben mcp add <name> --command <cmd>         Add a stdio server")
        _info("reuben mcp add <name> --preset <preset>       Add from a known preset")
        _info("reuben mcp remove <name>                      Remove a server")
        _info("reuben mcp list                               List configured servers")
        _info("reuben mcp test <name>                        Test connection")
        _info("reuben mcp configure <name>                   Toggle tools")
        _info("reuben mcp login <name>                       Re-authenticate OAuth")
        _info("reuben mcp reauth <name> | --all              Re-auth one or all OAuth servers")
        print()
