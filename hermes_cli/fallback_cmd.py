"""
reuben fallback — manage the fallback provider chain.
...
Subcommands:
  reuben fallback [list]   Show the current fallback chain (default when no subcommand)
  reuben fallback add      Pick provider + model via the same picker as `reuben model`,
                           then append the selection to the chain
  reuben fallback remove   Pick an entry to delete from the chain
  reuben fallback clear    Remove all fallback entries
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        ...
        # `reuben model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  reuben fallback add")
        print()
        return
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `reuben model`, then append the selection to the chain."""
    ...
    print("  `reuben model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `reuben fallback list` to view, or `reuben fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``reuben fallback [subcommand]``."""
