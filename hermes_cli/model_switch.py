...
_HERMES_MODEL_WARNING = (
    ...
    "for use with Reuben Agent. They lack the tool-calling capabilities "
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``reuben config set model.aliases.xxx``)
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'reuben model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'reuben doctor' — config issues detected:"
...
def list_authenticated_providers(
    ...
    *,
    ...
    for hermes_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        # /model picker sees the SAME list `reuben model` would build, with
    ...
    for pid, overlay in HERMES_OVERLAYS.items():
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            # `reuben model` picker deliberately shows ONLY the curated agentic
