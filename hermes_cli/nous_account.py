    ...
    *,
    ...
    if account_info is None:
        return (
            ...
            f"is unavailable. Run `reuben model` to refresh your login, or check "
    ...
    if not account_info.logged_in:
        ...
        return (
            f"Log in to Nous Portal to use {capability}: run `reuben model`. "
    ...
    if account_info.paid_service_access is None:
        ...
        if include_refresh_hint:
            detail += " Run `reuben model` to refresh your session."
    ...
    if reason == "account_missing":
        return (
            ...
            f"login, so {capability} is unavailable. Run `reuben model` to "
    ...
    if reason == "no_usable_credits" or account_info.paid_service_access is False:
        ...
        if include_refresh_hint and not account_info.fresh:
            message += " If you recently bought credits, run `reuben model` to refresh Hermes."
        return message
