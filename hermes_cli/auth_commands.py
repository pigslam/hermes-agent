...
def auth_add_command(args) -> None:
    ...
    if provider == "nous":
        ...
        # full device-code flow. This makes `reuben --profile <name>
    ...
    if provider == "openai-codex":
        ...
        # latest login: a second ``reuben auth add openai-codex`` overwrote
    ...
    raise SystemExit(f"`reuben auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `reuben auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `reuben auth` is called bare."""
