#!/usr/bin/env python3
"""
Reuben Agent CLI - Main entry point.

Usage:
    reuben                     # Interactive chat (default)
    reuben chat                # Interactive chat
    reuben gateway             # Run gateway in foreground
    reuben gateway start       # Start gateway as service
    reuben gateway stop        # Stop gateway service
    reuben gateway status      # Show gateway status
    reuben gateway install     # Install gateway service
    reuben gateway uninstall   # Uninstall gateway service
    reuben setup               # Interactive setup wizard
    reuben logout              # Clear stored authentication
    reuben status              # Show status of all components
    reuben cron                # Manage cron jobs
    reuben cron list           # List cron jobs
    reuben cron status         # Check if cron scheduler is running
    reuben doctor              # Check configuration and dependencies
    ...
    reuben version             Show version
    reuben update              Update to latest version
    reuben uninstall           Uninstall Reuben Agent
    reuben acp                 Run as an ACP server for editor integration
    reuben sessions browse     Interactive session picker with search

    reuben claw migrate --dry-run  # Preview migration without changes
"""
...
# When the user upgrades code via ``git pull`` (or ``reuben update``
...
# ``reuben update`` to recover.  Missing the bootstrap means UTF-8 stdio
...
def _set_process_title() -> None:
    """Set the process title to 'reuben' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``reuben tools`` or
      ...
      4. No-op on Windows (the .exe name is already ``reuben.exe``).
    """
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, CLI_COMMAND.encode("utf-8"), 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(CLI_COMMAND.encode("utf-8"))
...
def _suppress_mouse_residue_early() -> None:
    ...
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`reuben --tui … >log`, CI capture):
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``reuben --version`` before config/logging imports on Termux."""
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (reuben tools, reuben setup, reuben model) use
...
def _apply_profile_override() -> None:
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        """True once argv reaches `reuben mcp add ... --args <command argv>`.
    ...
    # after the subcommand (`reuben chat -p coder`), so keep scanning broadly.
    ...
    # `reuben profile use` and the gateway should honour that choice.
    # See issue #22502.
    ...
    # reserved ``gateway-default`` slot runs bare ``reuben gateway run`` to mean
...
def _print_tui_exit_summary(
    ...
    print(f"  {CLI_COMMAND} --tui --resume {target}")
    if title:
        print(f'  {CLI_COMMAND} --tui -c "{title}"')
    print()
...
def _restore_tui_workspace(tui_dir: Path) -> bool:
    ...
    files deleted in the working tree after ``reuben update`` (HEAD stays
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    print(
        ...
        "This usually means `reuben update` left tracked ui-tui files deleted.\n"
        "Recovery:\n"
        "  1. From the Hermes checkout, run `git restore -- ui-tui`\n"
        "  2. Run `npm install --silent --no-fund --no-audit --progress=false`\n"
        "  3. Retry `reuben --tui`\n"
        "If the checkout is still inconsistent, run `reuben update --force`.",
        file=sys.stderr,
    )
    sys.exit(1)
...
def _launch_tui(
    ...
    # in the user's shell would otherwise make a plain `reuben --tui` try to
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `reuben update` so
...
def _pin_kanban_board_env() -> None:
    ...
    (``reuben kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``reuben kanban boards switch`` from another session can flip the file
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print(f"Use '{CLI_COMMAND} sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    try:
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write(f"  \033[2mRun '{CLI_COMMAND} doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        print()
        print(
            f"It looks like {PRODUCT_NAME} isn't configured yet -- no API keys or providers found."
        )
        print()
        print(f"  Run:  {CLI_COMMAND} setup")
        print()
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print(f"You can run '{CLI_COMMAND} setup' at any time to configure.")
        sys.exit(1)
...
def cmd_whatsapp(args):
    ...
    # has no creds.json.  Every subsequent `reuben gateway` then paid a 30s
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: reuben gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `reuben gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  reuben gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with 'Reuben Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  reuben gateway")
        ...
        print()
        print("  Or install as a service: reuben gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'reuben whatsapp' to try again.")
...
def cmd_whatsapp_cloud(args):
    ...
    Distinct from ``reuben whatsapp`` (the Baileys bridge wizard) — the
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``reuben model``) and the setup wizard
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'reuben model' for "
                "available providers, or run 'reuben doctor' to diagnose config "
                "issues."
            )
...
# `reuben model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `reuben model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``reuben model`` flow, then route aux tasks to them here.
    """
    from hermes_cli.config import load_config
...
# fast paths like `reuben --version` and slash-command dispatch that never
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``reuben setup`` / ``reuben model``.
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `reuben setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            print(
                f"    Reuben will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def cmd_login(args):
    """Authenticate Reuben CLI with a provider."""
...
def cmd_slack(args):
    ...
    Dispatches ``reuben slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: reuben slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `reuben slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1
...
def cmd_security(args):
    """Dispatch `reuben security <subcmd>`."""
...
def cmd_backup(args):
    """Back up Reuben home directory to a zip file."""
...
def cmd_import(args):
    """Restore a Reuben backup from a zip file."""
...
def cmd_uninstall(args):
    """Uninstall Reuben Agent (or just the Chat GUI with --gui)."""
...
# even run ``reuben update`` again to roll forward. The post-pull syntax
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``reuben update --gateway`` so interactive prompts (stash restore,
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``reuben update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``reuben web``).
...
#   - ``reuben update`` can unconditionally call ``reuben desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``reuben desktop`` (interactive launch) skips the build when the
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``reuben update`` that pulled new source but hasn't built yet).
    """
...
def _force_adhoc_macos_signing(env: dict, *, source_mode: bool) -> bool:
    ...
    machine* (``reuben desktop --build-only`` → electron-builder ``--dir``).
...
def cmd_gui(args: argparse.Namespace):
    ...
    # `HERMES_DESKTOP_DISABLE_GPU=... reuben desktop` keeps working.
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  reuben gui")
            sys.exit(1)
    else:
        npm = None
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            if build_result.returncode != 0:
                ...
                print("    ELECTRON_MIRROR=<mirror-base-url> reuben desktop --force-build")
...
def _find_stale_dashboard_pids(
    *,
    ...
    """Return PIDs of ``reuben dashboard`` processes other than ourselves.

    ``reuben dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``reuben update`` replaces files on disk, the running
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    ...
    backend child process: when the desktop spawns ``reuben serve`` as
    ...
    patterns = [
        ...
        # The headless backend (`reuben serve`) is the same long-lived server
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `reuben update`.
    ...
    print("  Preview now:  reuben curator run --dry-run")
    print("  Pause it:     reuben curator pause")
    print(
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``reuben update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``reuben update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: reuben curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``reuben dashboard`` processes.

    Called at the end of ``reuben update`` (default ``reason``) and also
    from ``reuben dashboard --stop`` (which overrides ``reason``).  The
    ...
    if killed:
        ...
        print("    reuben dashboard --port <port>")
...
def _update_via_zip(args):
    """Update Reuben Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `reuben update "
            f"--branch {branch}`, or update against main with `reuben update`."
        )
        sys.exit(1)
    zip_url = (
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: reuben skills list-modified  "
...
# =========================================================================
# Fork detection and upstream management for `reuben update`
# =========================================================================
...
def _invalidate_update_cache():
    ...
    ``reuben update``, every profile is now current.
    """
    homes = []
...
# Install-scoped breadcrumb dropped right before ``reuben update`` mutates the
...
def _recover_from_interrupted_install() -> None:
    """Finish a dependency install that a prior ``reuben update`` left half-done.
    ...
    protocol stream (``reuben acp`` speaks JSON-RPC on stdout) must never get
    ...
    try:
        ...
        print(
            "⚠ A previous `reuben update` was interrupted mid-install — "
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``reuben update`` is still progressing even if pip/uv itself is silent.
    """
...
def _detect_concurrent_hermes_instances(
    ...
    so during ``reuben update`` the user-invoked process and the desktop's
    ...
    chain are excluded so the running ``reuben update`` invocation never
    ...
    # file lock) is the parent. Without excluding it, every ``reuben update``
...
def _format_concurrent_instances_message(
    ...
    lines.append("  stop the gateway (`reuben gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `reuben update --force` if you've already")
...
def _quarantine_running_hermes_exe(
    ...
    when ``reuben update`` runs, ``hermes.exe`` IS the live process, and uv
    ...
    for shim in _hermes_exe_shims(scripts_dir):
        ...
        print(
            ...
            "gateway, or pause AV scanning, then re-run `reuben update`."
        )
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``reuben update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `reuben update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    # hours later inside ``reuben update``'s desktop-rebuild or skill-sync
...
def _verify_console_scripts_installed(
    ...
    *,
    ...
    ``hermes.exe`` shim is locked during ``reuben update``, or when uv/distlib
    ...
    try:
        ...
        print(
            "  ⚠ Entry point repair failed; try `reuben update --force` after "
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``reuben update``, not the venv we just wrote
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `reuben update` output above.")
        return
    ...
    try:
        ...
        print(
            ...
            "Run `reuben update --force` after closing other hermes processes."
        )
        return
...
def _update_node_dependencies() -> None:
    ...
    # need desktop during `reuben update`, so we install root-only first
    ...
    # hung. The chatty npm-deprecation noise during `reuben update` comes from
...
class _UpdateOutputStream:
    """Stream wrapper used during ``reuben update`` to survive terminal loss.
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``reuben update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``reuben update`` in an SSH session or a terminal
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``reuben update --gateway``) the update is already
...
def _log_only_write(text: str) -> None:
    ...
    During ``reuben update`` ``sys.stdout`` is an ``_UpdateOutputStream`` that
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``reuben update --check``: fetch and report without installing.
    ...
    if method == "docker":
        ...
        # same long-form ``docker pull`` guidance ``reuben update`` (apply
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``reuben update`` without requiring a reinstall.  The
    ...
    ``reuben`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/reuben.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v reuben'`` already resolves.  Idempotent.
    """
    if sys.platform != "linux":
        return
    try:
    ...
    # /usr/local/bin/reuben, code at /usr/local/lib/hermes-agent).
    fhs_link = Path("/usr/local/bin/reuben")
    legacy_fhs_link = Path("/usr/local/bin/hermes")
    if not (
        fhs_link.is_symlink()
        or fhs_link.exists()
        or legacy_fhs_link.is_symlink()
        or legacy_fhs_link.exists()
    ):
        return
    ...
    path_comment = (
        "# Reuben Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    ...
    HERMES_HOME directories.  The ``--backup`` flag on ``reuben update``
    ...
    print(f"  Restore:  reuben import {out_path}")
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    if unmapped_pids:
        ...
        if respawnable < len(unmapped_pids):
            ...
            print("    Restart manually after update: reuben gateway run")
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    ``reuben gateway start`` uses (``gateway_windows._spawn_detached``).
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``reuben update`` sees a clean tree instead of autostashing every run.
...
def cmd_update(args):
    """Update Reuben Agent to the latest version.
    ...
    if is_managed():
        managed_error("update Reuben Agent")
        return
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `reuben update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
...
def _cmd_update_pip(args):
    ...
    print("✓ Update complete! Restart reuben to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # every user who ran ``reuben update`` for the 7 minutes between
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``reuben update`` again later once a fix lands.")
                    else:
        ...
        # build.  ``reuben desktop --build-only`` uses the content-hash stamp
        ...
        # never run ``reuben desktop`` shouldn't be forced into a full
        # Electron build by ``reuben update``.
        desktop_dir = PROJECT_ROOT / "apps" / "desktop"
        ...
        if (desktop_dir / "package.json").exists() and find_node_executable("npm") and has_desktop_app:
            ...
            if build_result.returncode != 0:
                build_result = _run_logged_subprocess(_desktop_build_cmd, cwd=PROJECT_ROOT)
            if build_result.returncode != 0:
                print("  ⚠ Desktop build failed (non-fatal; run `reuben desktop` to retry)")
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'reuben config migrate' to retry.")
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: reuben config migrate")
            else:
                print()
                print("Skipped. Run 'reuben config migrate' later to configure.")
        else:
        ...
        # check `reuben curator status`. Self-stamps after printing so it
        ...
        # don't have it. Tying the refresh to ``reuben update`` gives users a
        ...
        # When running as ``reuben update --gateway`` (spawned by the gateway's
        ...
        try:
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            if _manage_cmd is None:
                                print(
                                    ...
                                    f"    To let `reuben update` restart it automatically, allow\n"
                            ...
                            # path in `reuben gateway restart`
                        ...
                        print(
                            ...
                            f"Check the gateway with: reuben gateway status"
                        )
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: reuben gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: reuben -p <profile> gateway run  for each profile)"
                        )
        ...
        # every `reuben update` surfaces the issue until the user migrates.
        try:
            from hermes_cli.gateway import (
            ...
            if supports_systemd_services() and has_legacy_hermes_units():
                ...
                print()
                print("    reuben gateway migrate-legacy")
                print()
        ...
        print("  reuben model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``reuben -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `reuben profile` — show current profile status
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    ...
                    print(f"Alias:          {alias_display} → reuben -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  reuben profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     reuben -p {name} chat")
                else:
        ...
        if dist_name:
            ...
            print(f"  (run `reuben profile info {name}` for full manifest)")
        if alias_name:
            is_windows = sys.platform == "win32"
            wrapper = _get_wrapper_dir() / (f"{alias_name}.bat" if is_windows else alias_name)
            print(f"Alias:   {alias_name} → reuben -p {name}  ({wrapper})")
        print()
        ...
        try:
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  reuben -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      reuben -p {plan.manifest.name} chat")
        ...
        try:
            ...
            if plan.has_cron:
                print(
                    ...
                    f"reuben -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
...
def _report_dashboard_status() -> int:
    """Print ``reuben dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``reuben dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No reuben dashboard processes running.")
        return 0

    print(f"{len(pids)} reuben dashboard process(es) running:")
    for pid in pids:
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    ``reuben dashboard register`` for OAuth.
    ...
    print("    [2] OAuth via Nous Portal (run `reuben dashboard register`)")
    print("    [3] Cancel")
    print()
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No reuben dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `reuben update`.
...
def cmd_logs(args):
    """View and filter Reuben log files."""
...
# so that in ``reuben -m gpt5 chat``, ``gpt5`` is correctly skipped as a
...
def _first_positional_argv() -> str | None:
    ...
    ``reuben -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `reuben --tui` is the hot path on phones. The full parser setup imports
...
def cmd_acp(args):
    """Launch Reuben Agent as an ACP server."""
    try:
...
def main():
    ...
    # ``reuben update`` runs on Windows. Silent no-op on non-Windows or when
    ...
    # Self-heal a venv left half-built by an interrupted ``reuben update``
    ...
    # ``reuben skills install update``) merely defers recovery one launch;
    # under-matching (missing ``reuben -p work update``) would race a recovery
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `reuben model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    whatsapp_cloud_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "required). Distinct from `reuben whatsapp` which sets up "
    ...
    # subcommand — ``reuben --help``, ``reuben version``, ``reuben logs``,
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Linux.\n\n"
            "Use `reuben computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `reuben tools` runs when you first\n"
            "enable the Computer Use toolset, and is a stable target\n"
            "for re-running the install if it didn't fire (e.g. when\n"
            "toggling the toolset on a returning-user setup).\n\n"
            "Use `reuben computer-use doctor` to run cua-driver's\n"
            ...
            if path:
                ...
                try:
                    ...
                    if st and st.get("update_available"):
                        ...
                        print("    Run: reuben computer-use install --upgrade")
                    elif st:
                        print("  ✓ Up to date.")
                    else:
                        # Older driver (no check-update verb) or offline.
                        print("  Refresh to latest: reuben computer-use install --upgrade")
                except Exception:
                    print("  Refresh to latest: reuben computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: reuben computer-use install")
            return
        if action == "doctor":
        ...
        if action == "permissions":
            ...
            if perms_action == "status":
                ...
                if not st["platform_supported"]:
                    ...
                    sys.exit(1)
                if not st["installed"]:
                    print("cua-driver: not installed. Run: reuben computer-use install")
                    sys.exit(1)
                ...
                if st["can_grant"]:  # macOS TCC permissions
                    ...
                    if not st["ready"]:
                        print("  Grant: reuben computer-use permissions grant")
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch reuben --resume <id> by replacing the current process
    ...
    # run `reuben desktop` from a terminal, so the canonical name needs
    ...
    # e.g. ``reuben -c Pokemon Agent Dev`` → ``reuben -c 'Pokemon Agent Dev'``
    ...
    # fails (e.g. 'reuben -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (reuben hooks list, cron
