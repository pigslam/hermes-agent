"""`reuben checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.hermes/checkpoints/``.  Actions:

    reuben checkpoints               # same as `status`
    reuben checkpoints status        # total size, project count, breakdown
    reuben checkpoints list          # per-project checkpoint counts + workdir
    reuben checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    reuben checkpoints clear [-f]    # nuke the entire base (asks first)
    reuben checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    reuben checkpoints
    reuben checkpoints prune --retention-days 3 --max-size-mb 200
    reuben checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: reuben checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``reuben checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `reuben checkpoints` → status
