"""``reuben portal`` — the human-readable entry point for Nous Portal.

Running ``reuben portal`` with no subcommand performs the one-shot Portal
...
``reuben setup --portal``, and runs the same Nous flow as the first-time quick
setup.
...
already in ``hermes auth`` or ``reuben tools``. It's the onboarding + discovery
...
def _cmd_status(args) -> int:
    ...
    if provider == "nous":
        ...
        print(f"  Model:   currently {provider} (switch with `reuben model`)")
...
def _cmd_tools(args) -> int:
    ...
    if not features.nous_auth_present:
        print(color("  Not logged into Nous Portal — sign in with `reuben portal`.", Colors.YELLOW))
        print()
...
def _cmd_login(args) -> int:
    ...
    oauth`. It reuses the exact wiring behind `reuben setup --portal` (which in
...
def portal_command(args) -> int:
    """Top-level dispatch for `reuben portal <subcommand>`."""
    sub = getattr(args, "portal_command", None)
    if sub in {None, "", "login"}:
        # Default to the one-shot onboarding — `reuben portal` is the
    ...
    print("Run `reuben portal -h` for usage.", file=sys.stderr)
    return 1


def add_parser(subparsers) -> None:
    """Register `reuben portal` on the given argparse subparsers object."""
    portal_parser = subparsers.add_parser(
        "portal",
        help="Set up Nous Portal (login, model pick, Tool Gateway); see also `portal info`",
        description=(
            "Run `reuben portal` with no subcommand to log in to Nous Portal "
            ...
            "nous --type oauth`, identical to `reuben setup --portal`). "
