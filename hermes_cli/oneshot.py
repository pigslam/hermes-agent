...
configured for "cli" in `reuben tools`.
...
def _validate_explicit_toolsets(toolsets: object = None) -> tuple[list[str] | None, str | None]:
    ...
    try:
        ...
        return None, f"{CLI_COMMAND} -z: failed to validate --toolsets: {exc}\n"
    ...
    if any(name in {"all", "*"} for name in built_in):
        ...
        if ignored:
            sys.stderr.write(
                f"{CLI_COMMAND} -z: --toolsets all enables every toolset; "
    ...
    if unknown:
        sys.stderr.write(f"{CLI_COMMAND} -z: ignoring unknown --toolsets entries: {', '.join(unknown)}\n")
    if disabled:
        sys.stderr.write(
            f"{CLI_COMMAND} -z: ignoring disabled MCP servers (set enabled: true in config.yaml to use): "
            f"{', '.join(disabled)}\n"
        )

    if not valid:
        return None, f"{CLI_COMMAND} -z: --toolsets did not contain any valid toolsets.\n"
...
def run_oneshot(
    ...
    if provider and not ((model or "").strip() or env_model_early):
        sys.stderr.write(
            f"{CLI_COMMAND} -z: --provider requires --model (or HERMES_INFERENCE_MODEL). "
    ...
    if failure is not None:
        ...
        if isinstance(failure, (KeyboardInterrupt, SystemExit)):
            raise failure
        real_stderr.write(f"{CLI_COMMAND} -z: agent failed: {failure}\n")
        real_stderr.flush()
        return 1
    ...
    if not (response or "").strip():
        real_stderr.write(f"{CLI_COMMAND} -z: no final response was produced; treating the run as failed.\n")
        real_stderr.flush()
        return 1
...
def _create_session_db_for_oneshot():
    """Best-effort SessionDB for ``reuben -z`` / oneshot mode.
