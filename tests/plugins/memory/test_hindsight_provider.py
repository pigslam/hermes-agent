...
class TestConfig:
    ...
    def test_default_values(self, provider):
        ...
        assert provider._retain_context == "conversation between Reuben Agent and the User"
...
class TestSyncTurn:
    def test_sync_turn_retains_metadata_rich_turn(self, provider_with_config):
        ...
        item = call_kwargs["items"][0]
        assert item["context"] == "conversation between Reuben Agent and the User"
    ...
    def test_sync_turn_uses_aretain_batch(self, provider):
        ...
        assert call_kwargs["retain_async"] is True
        assert len(call_kwargs["items"]) == 1
        assert call_kwargs["items"][0]["context"] == "conversation between Reuben Agent and the User"
