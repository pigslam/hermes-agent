...
def test_looks_like_human_speaker():
    ...
    for s in ("", "   ", "Unknown", "unknown", "You", "you", "Reuben Agent", "reuben agent"):
        assert not _looks_like_human_speaker(s, "Reuben Agent"), f"{s!r} should NOT be human"
    # Real names → human (barge-in)
    for s in ("Alice", "Bob Lee", "@teknium"):
        assert _looks_like_human_speaker(s, "Reuben Agent"), f"{s!r} SHOULD be human"
