...
def test_build_welcome_banner_title_is_hyperlinked_to_release():
    ...
    assert "Reuben Agent v" in raw, "Version label missing from title"
...
def test_build_welcome_banner_uses_reuben_brand_and_portrait():
    import io
    from unittest.mock import patch as _patch
    import hermes_cli.banner as _banner
    import model_tools as _mt
    ...
    raw = buf.getvalue()
    assert "REUBEN-AGENT" in raw
    assert "Reuben Agent v" in raw
