...
# 5. Symlinks the 'reuben' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}Reuben Agent Setup${NC}"
echo ""
...
# ============================================================================
# PATH setup — symlink reuben into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up reuben command..."

REUBEN_BIN="$SCRIPT_DIR/venv/bin/reuben"
HERMES_BIN="$SCRIPT_DIR/venv/bin/hermes"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$REUBEN_BIN" "$COMMAND_LINK_DIR/reuben"
ln -sf "$HERMES_BIN" "$COMMAND_LINK_DIR/hermes"
echo -e "${GREEN}✓${NC} Symlinked reuben → $COMMAND_LINK_DISPLAY_DIR/reuben"
echo -e "${GREEN}✓${NC} Symlinked hermes compatibility alias → $COMMAND_LINK_DISPLAY_DIR/hermes"
...
echo "Other commands:"
echo "  reuben status        # Check configuration"
if is_termux; then
    echo "  reuben gateway       # Run gateway in foreground"
else
    echo "  reuben gateway install # Install gateway service (messaging + cron)"
fi
echo "  reuben cron list     # View scheduled jobs"
echo "  reuben doctor        # Diagnose issues"
echo ""
