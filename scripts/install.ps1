# ============================================================================
# Reuben Agent Installer for Windows
# ============================================================================
...
function Write-Banner {
    ...
    Write-Host "|             * Reuben Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
...
function Install-Venv {
    ...
    if (Test-Path "venv") {
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running reuben/hermes processes before recreating venv..."
            # The launcher CLI (reuben.exe/hermes.exe) plus its child tree.
            & taskkill /F /T /IM reuben.exe /FI "PID ne $myPid" 2>$null | Out-Null
...
"@ 2>$null
    ...
    # users hit and lazy-import errors from `reuben dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `reuben dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `reuben dashboard` should now work."
            } else {
...
function Set-PathVariable {
    Write-Info "Setting up reuben command..."
    ...
    # Add the venv Scripts dir to user PATH so reuben is globally available.
    # On Windows, the reuben.exe in venv\Scripts\ has the venv Python baked in.
    ...
    Write-Success "reuben command ready"
}
...
function Install-NodeDeps {
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'reuben setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'reuben setup'."
        return
    }
...
function Start-GatewayIfConfigured {
    ...
    $hermesCmd = "$InstallDir\venv\Scripts\reuben.exe"
    if (-not (Test-Path $hermesCmd)) {
        $fallbackCmd = "$InstallDir\venv\Scripts\hermes.exe"
        if (Test-Path $fallbackCmd) {
            $hermesCmd = $fallbackCmd
        } else {
            $hermesCmd = "reuben"
        }
    }
    ...
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        ...
        Write-Info "Running 'reuben whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: reuben gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            ...
            Write-Warn "Failed to start gateway. Run manually: reuben gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: reuben gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host ""
    Write-Host "   reuben              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   reuben setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   reuben config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   reuben config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   reuben gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   reuben update       " -NoNewline -ForegroundColor Green
